...
void ssa_dependency_grapht::output(std::ostream &out) const
{
  ...
  {
    out << "Node#" << index << "; info: "
        << from_expr(ns, "", depnodes_map[index].node_info) << "\n";
    out << "Node#" << index << "; -> Used Symbols: ";
    for(find_symbols_sett::const_iterator u_it=
          depnodes_map[index].used_symbols.begin();
    ...
    for(find_symbols_sett::const_iterator m_it=
          depnodes_map[index].modified_symbols.begin();
    ...
    out << "Predecessors:\n";
    for(ssa_dependency_grapht::annotated_predecessorst::const_iterator p_it=
          depnodes_map[index].predecessors.begin();
...
void ssa_dependency_grapht::create(
  const local_SSAt &SSA, ssa_inlinert &ssa_inliner, bool entry)
{
  depnodet sink_node;
      ...
      n_it!=SSA.nodes.end(); n_it++)
  {
    ...
    {
      bool enabled=true;
      const irep_idt &enable=
        to_symbol_expr(node.enabling_expr).get_identifier();
      for(size_t i=0; i<SSA.enabling_exprs.size(); ++i)
      {
        if(SSA.enabling_exprs[i].id()==ID_not)
        {
          if(to_symbol_expr(SSA.enabling_exprs[i].op0()).get_identifier()==
             enable)
          {
            enabled=false;
            break;
        ...
        e_it!=node.equalities.end(); e_it++)
    {
      ...
      // loop-head select
      // TODO: this is an ugly hack (this can be changed
      //   as soon as unwindable_local_SSA provides
      //   smooth renaming with odometers)
...
#if 0
    // collecting symbols from assumptionst
    //   and populating dependency graph nodes
    for(local_SSAt::nodet::assumptionst::const_iterator a_it=
          node.assumptions.begin();
    ...
    // collecting symbols from function_callst
    //   and populating dependency graph nodes
        ...
        f_it!=node.function_calls.end(); f_it++)
    {
      ...
      if(ssa_db.exists(fname))
      {
        ...
        ssa_inliner.get_bindings(
          SSA, fSSA, n_it, f_it, bindings_in, bindings_out, counter);
        ...
        s_it=all_ssa_symbols.begin();  s_it!=all_ssa_symbols.end(); s_it++)
  {
    ...
    {
      if(depnodes_map[m_index].modified_symbols.find(*s_it)!=
         depnodes_map[m_index].modified_symbols.end())
      {
        for(unsigned u_index=0; u_index<depnodes_map.size(); u_index++)
        {
          if(m_index!=u_index)
          {
            if(depnodes_map[u_index].used_symbols.find(*s_it)!=
               depnodes_map[u_index].used_symbols.end())
            {
  ...
  else
  {
    for(unsigned index=1; index<depnodes_map.size()-1; index++)
    {
      ...
      if(depnodes_map[index].predecessors.empty())
      {
        ...
        annotated_predecessort def_pred;
        // successors of the default predecessors
        def_pred.predecessor_node_index=top_node_index;
      ...
      if(depnodes_map[index].successors.empty())
      {
        ...
        annotated_predecessort pred_def;
        // predecessors of the default successor
