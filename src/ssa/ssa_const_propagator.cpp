...
void ssa_const_propagatort::operator()(
  std::list<exprt> &dest, const local_SSAt &src)
{
  ...
  while(values.iterate)
  {
        ...
        n_it!=src.nodes.end(); n_it++)
    {
      ...
      // if src.enabling_exprs does not have a true in the end,
      //   then also continue
          ...
          e_it!=node.equalities.end(); e_it++)
      {
        ...
        while(temprhs.id()==ID_if)
        {
          exprt simp_guard;
#ifdef DEBUG
          std::cout << "guard:      " << from_expr(src.ns, "", temprhs.op0())
                    << std::endl;
#endif
          simp_guard=simplify_expr(temprhs.op0(), src.ns);
#ifdef DEBUG
          std::cout << "simplified: " << from_expr(src.ns, "", simp_guard)
                    << std::endl;
#endif
      ...
      ++it)
  {
    dest.push_back(
      equal_exprt(
        symbol_exprt(it->first, it->second.type()), it->second));
  }
}
...
void ssa_const_propagatort::valuest::set_to(
